...
const Input = props => {
  ...
  switch (props.elementType) {
    ...
    case 'heading':
      ...
      break;
    default:
      inputElement = <Form.Control className={inputClasses.join(' ')} {...props.elementConfig} value={props.value} onChange={props.changed} />;
  }

  const helpers = props.fieldHelper ?
    props.fieldHelper.map((helperText, i) => {
      return <Form.Text key={i}>{helperText}</Form.Text>;
    })
    : null;

  const fieldLabel = props.fieldLabel && props.elementType !== 'heading' ?
